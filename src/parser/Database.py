...
from src.parser.error.DatalogError import *
...
class Database(object):
    ...
    def _evaluate_or(self, or_, variables, facts, rules):
        ...
        new_varlist = []
        for child in or_.children:
            vars = self._process_literal(child, variables, facts, rules)
            for var_dict in vars:
                if var_dict not in new_varlist:
                    new_varlist.append(var_dict)
        return new_varlist
    ...
    def _follow_sentence(self, sentence, visited):
        if sentence.is_not():
            ...
            if len(sentence.children) == 2:
                a, b = [x.predicate for x in sentence.children]
                if self._find_neg_cycle(a, visited + [a]) or \
                        self._find_neg_cycle(b, visited + [b]):
                    return True
            elif len(sentence.children) == 3:
                a, b, c = [x.predicate for x in sentence.children]
                if self._find_neg_cycle(a, visited + [a]) or \
                        self._find_neg_cycle(b, visited + [b]) or \
                        self._find_neg_cycle(c, visited + [c]):
                    return True
            elif len(sentence.children) == 4:
                a, b, c, d = [x.predicate for x in sentence.children]
                if self._find_neg_cycle(a, visited + [a]) or \
                        self._find_neg_cycle(b, visited + [b]) or \
                        self._find_neg_cycle(c, visited + [c]) or \
                        self._find_neg_cycle(d, visited + [d]):
                    return True
        else:
